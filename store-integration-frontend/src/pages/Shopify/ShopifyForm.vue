...
export default {
  ...
  setup() {
    const loader = ref("");
    const placeholder = ref("Access Token");
    ...
    // 유효성 체크후, 성공시 키값 일부 노출
    const validateKey = async () => {
      validateBtnPressed.value = true;
      const customerId = getCustomerId();
      try {
        loader.value = "true";
        const response = await axios.get(`${customerId}/shopify/access-token`, {
          params: {
        ...
        loader.value = "";
        placeholder.value =
          response.data.data.accessToken.slice(0, 10) + "*************";
