<template>
  <v-col class="container" style="width : 900px; display: inline-block;">
    <v-img :src="SVG.RakutenLogo" width="260px" height="80px"></v-img>
    <PageTitle
      title="스토어 연동 절차"
    ...
    <RakutenManual :manualList="manualList" />
...
<script>
import { getRakutenManual } from "@/library/getManual";
import { computed, onMounted, ref } from "vue";
import { useStore } from "vuex";
import PageTitle from "@/components/PageTitle.vue";
import RakutenManual from "@/pages/rakuten/RakutenManual.vue";
import RakutenForm from "@/pages/rakuten/RakutenForm.vue";
...
export default {
  ...
  setup() {
    ...
    onBeforeRouteLeave((to, from, next) => {
      if (to.fullPath === "/SetAlias" && from.fullPath === "/Rakuten") {
        vuexStore.dispatch("selectStore/updateSelectedStore", "Rakuten");
    ...
    onMounted(() => {
      ...
      manualList.value = getRakutenManual();
    });
